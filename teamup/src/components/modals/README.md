...
```mermaid
graph TD
    A[Пользователь входит] --> B{isProfileCompleted?}
    B -->|false| C[Показать модалку]
    B -->|true| D[Пропустить]
    C --> E[Пользователь заполняет форму]
    E --> F[Валидация]
    F -->|Ошибки| E
    F -->|OK| G[Отправка на API]
    G -->|Успех| H[Обновить Zustand]
    H --> I[Закрыть модалку]
    H --> J[isProfileCompleted = true]
    G -->|Ошибка| K[Показать ошибку]
    K --> E
```
